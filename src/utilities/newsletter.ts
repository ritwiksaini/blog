...
/**
 * The frame both emails render into, so the welcome message and the
 * announcements are recognisably the same correspondence.
 *
 * Deliberately austere: one column, no images, no tracking pixel, no button.
 * A hero image and a coloured call-to-action are the clearest signal a spam
 * filter has that something is a bulk mailing rather than a letter, and these
 * should read as a letter.
 *
 * Every style is inline. Gmail strips <style> blocks in several contexts, and
 * a message that depends on them arrives unstyled.
 */
const shell = ({ body, footer }: { body: string; footer: string }) =>
...
const footerLink = (href: string, label: string) =>
  `<a href="${href}" style="color:${INK_MUTED};text-decoration:underline;">${label}</a>`

/**
 * Sent the moment someone subscribes.
 *
 * Single opt-in: this arrives as the notification rather than as a gate, which
 * makes the unsubscribe line load-bearing rather than decorative. Anyone can
 * type someone else's address into the form, so this email is the only thing
 * standing between that and a spam complaint. It says plainly what happened and
 * how to undo it, and the footer is worded for a person who did NOT sign up
...
export const welcomeEmail = (base: string, unsubscribeToken: string) => {
  ...
  const text = [
    'Hello,',
    '',
    'You are now subscribed to new posts by Ritwik Saini.',
    '',
    'Posts arrive when one is finished, usually with a note on why it was worth',
    'writing. Nothing else is sent.',
    '',
  ...
  const html = shell({
    ...
    <p style="margin:0 0 16px;">You are now subscribed to new posts by Ritwik Saini.</p>
    <p style="margin:0 0 28px;">Posts arrive when one is finished, usually with a note on why it was worth writing. Nothing else is sent.</p>
    <p style="margin:0 0 32px;"><a href="${base}" style="color:${ACCENT};text-decoration:underline;">Read the archive</a></p>`,
    footer: `      If you did not sign up for this, ${footerLink(unsubscribe, 'unsubscribe')} and you will not hear from this address again.`,
  })
 ...
 *
 * Opens with the greeting and intro for the same reason the welcome email
 * does: this should read as something a person wrote, not a notification.
 */
